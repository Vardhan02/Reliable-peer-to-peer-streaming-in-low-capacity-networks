...
private:
 ...
 //  int       m_nParent;
  ...
  // UstreamHelper. Report is not static methods
  MeshHelper mesh;
private:
...
{
  CommandLine cmd;
  cmd.AddValue ("R-size", "Number of root nodes in a row grid. [6]", m_nRoot);
  //cmd.AddValue ("P-size", "Number of parent nodes in a row grid. [4]", m_nParent);
  //cmd.AddValue ("C-size", "Number of children nodes in a row grid. [16]", m_nChildren);
 cmd.AddValue ("y-size", "Number of rows in a grid. [6]", m_ySize);
  ...
  cmd.Parse (argc, argv);
  NS_LOG_DEBUG ("Grid:" << m_nRoot << "*" << m_ySize);
...
{ 
  ...
  mobility.SetPositionAllocator ("ns3::GridPositionAllocator",
                                 ...
                                 "GridWidth", UintegerValue (m_nRoot),
                                // "GridWidth", UintegerValue (m_nParent),
                                 //"GridWidth", UintegerValue (m_nChildren),
...
UdpServerHelper Server (port3);
 ...
 UdpTraceClientHelper video (interfaces.GetAddress (0), port3,  "");
...
// video.SetAttribute ("Interval", TimeValue (Seconds (m_packetInterval)));
  ...
  ApplicationContainer clientApps = video.Install (nodes.Get (m_nRoot*m_ySize-1));
...
MeshTest::Run ()
{
  ...
  //Gnuplot parameters for throughput
    ...
    std::string fileNameWithNoExtension = "Throughput_";
...
void ThroughputMonitor (FlowMonitorHelper *fmhelper, Ptr<FlowMonitor> flowMon,Gnuplot2dDataset DataSet)
	{
      ...
      // packetLoss=(stats->second.lostPackets);
			// updata gnuplot data
   ...
   //if(flowToXml)
      {
	flowMon->SerializeToXmlFile ("testing1.xml", true, true);
      }
