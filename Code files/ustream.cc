...
{ 
   ...
   */
  rootNodes.Create (m_ySize*m_xSize);
//parentNodes.Create (m_ySize*m_xSize);
  ...
  rootDevices = mesh.Install (wifiPhy, rootNodes);
...
MeshTest::InstallInternetStack ()
{
  ...
  internetStack.Install (rootNodes);
...
uint16_t port3 = 11; 
  ...
  ApplicationContainer serverApps = Server.Install (rootNodes.Get (0));
  ...
  ApplicationContainer clientApps = video.Install (rootNodes.Get (m_xSize*m_ySize-1));
...
{
 /* unsigned n (0);
  for (NetDeviceContainer::Iterator i = rootDevices.Begin (); i != rootDevices.End (); ++i, ++n)
    {
