...
//NodeContainer parentPeer;
 ...
 //NetDeviceContainer meshPDevices;
...
childrenNodes(2),
  ...
  //m_parent ("ff:ff:ff:ff:ff:ff"),
  //m_children("ff:ff:ff:ff:ff:ff")
{
}
void
MeshTest::Configure (int argc, char *argv[])
{
 ...
 // cmd.AddValue ("parent", "Mac address of parent mesh point in HWMP", m_parent);
 //cmd.AddValue ("children", "Mac address of children mesh point in HWMP", m_children);
  cmd.Parse (argc, argv);
...
//nodes.Create (1000);
 ...
 //if (!Mac48Address (m_parent.c_str ()).IsBroadcast ())
   // {
     // mesh.SetStackInstaller (m_stack, "Parent", Mac48AddressValue (Mac48Address (m_parent.c_str ())));
    //}
  //else
    //{
      //If root is not set, we do not use "parent" attribute, because it
      //is specified only for 11s
      //mesh.SetStackInstaller (m_stack);
   // }

 //if (!Mac48Address (m_children.c_str ()).IsBroadcast ())
   // {
     // mesh.SetStackInstaller (m_stack, "Children", Mac48AddressValue (Mac48Address (m_parent.c_str ())));
  //  }
  //else
    //{
      //If root is not set, we do not use "Root" attribute, because it
...
// meshPDevices = mesh.Install (wifiPhy, parentPeer);
...
for (uint32_t i = 0; i < rootNodes; ++i)
    {
      ...
      for (uint32_t j = 0; j < newParentNodes.GetN (); ++j)
        {
          subnetAlloc->Add (Vector (0.0, j*2 + 2, 0.0));
        }
...
for (uint32_t i = 0; i < parentNodes; ++i)
    {
      ...
      for (uint32_t j = 0; j < newChildrenNodes.GetN (); ++j)
        {
          subnetAlloc->Add (Vector (0.0, j*4 + 4, 0.0));
        }
...
MeshTest::InstallInternetStack ()
{
 ...
 //internetStack.Install (parentPeer);
 // Ipv4AddressHelper address1;
 // address1.SetBase ("192.168.3.0", "255.255.255.0");
  //interfaces1 = address1.Assign (meshPDevices);
...
//UdpServerHelper Server (port3);
  ...
  UdpTraceClientHelper video (remoteAddr, port,  "");
