...
NS_LOG_COMPONENT_DEFINE ("multirate-new");
...
private:
  ...
  void MeshInstall (NodeContainer c);

  Gnuplot2dDataset m_output, dataset;
...
Experiment::SelectSrcDest (NodeContainer c)
{
  ...
  uvSrc->SetAttribute ("DataRate", StringValue ("100Mbps"));
  uvSrc->SetAttribute ("Delay", TimeValue (MilliSeconds (1)));
  ...
  uvDest->SetAttribute ("DataRate", StringValue ("50Mbps"));
  //uvDest->SetAttribute ("Delay", TimeValue (MilliSeconds (2)));
...
void
Experiment::MeshInstall (NodeContainer c)
{
...
MeshHelper mesh;
  ...
  NetDeviceContainer devices = mesh.Install (phy, c);
  //devices = mesh.Install (phy, c);
}
...
Experiment::ApplicationSetup (Ptr<Node> client, Ptr<Node> server, double start, double stop)
{
  ...
  apps.Stop (Seconds (stop));*/
   ...
   uint32_t MaxPacketSize = 2000;  // Back off 20 (IP) + 8 (UDP) bytes from MTU
  UdpTraceClientHelper trace (ipv4AddrServer, port, "");
  trace.SetAttribute ("MaxPacketSize", UintegerValue (MaxPacketSize));
  //trace.SetConstantRate ("DataRate", StringValue ("5Mbps"));
  //trace.SetAttribute ("Delay", TimeValue (MilliSeconds (2)));
...
std::string m_stack; ///< stack
 ...
 m_nIfaces (1),
  ...
  //PointToPointHelper p2p = pointToPoint;
 ...
 //NetDeviceContainer devices = wifi.Install (phy, mac, c);
  ...
  //wifi.SetDeviceAttribute ("DataRate", StringValue ("5Mbps"));
  //wifi.SetChannelAttribute ("Delay", TimeValue (MilliSeconds (2))); */
  NetDeviceContainer devices = wifi.Install (phy, mac, c);
  //devices = mesh.Install (phy, c);
  //devices = p2p.Install(c);
...
// Instantiate the plot and set its title for throughput.
    ...
    gnuplot.SetLegend ("Flow", "STS-Throughput(Mbps)");
...
//meanDelay=((stats->second.delaySum.GetSeconds()) / stats->second.rxPackets );
 meanJitter=((stats->second.jitterSum.GetSeconds())/ ((stats->second.rxPackets)-1));
   ...
   // startDelay =  (stats->second.timeFirstTxPacket.GetSeconds())/ (stats->second.delaySum.GetSeconds()) ;
			// updata gnuplot data
            DataSet.Add((double)Simulator::Now().GetSeconds(),(double) meanJitter);
